...
import { useRotate } from '~/composables/utils/rotate'
...
const breadcrumbs = computed<BreadcrumbListItem[]>(() => [
  ...
  { label: data.value?.title ?? '記事', route: { name: `tech-${route.params.slug}` } },
])
      ...
      <div class="space-y-4">
        ...
        >
          <li
            ...
            :key="index"
          >
            <p class="text-sm border border-gray-400 opacity-80 font-bold rounded-full leading-none py-1 px-2">
              #{{ tag }}
            </p>
          </li>
        </ul>
